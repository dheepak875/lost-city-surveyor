        ...
        if (this.economy.spend('terraquest')) {
            ...
            for (let r = rStart; r < rEnd; r++) {
                for (let c = cStart; c < cEnd; c++) {
                    ...
                    const structure = this.grid.checkStructureFound(r, c);
                    if (structure) {
                        newDiscoveries++;
                        this.economy.reward('structure');
                    }
            ...
            if (newDiscoveries > 0) {
                audio.discovery();
                this.uiCallbacks.logMessage(`TerraQuest Hybrid Rover uncovered ${newDiscoveries} artifacts!`, "success");
                this.uiCallbacks.updateTargets(this.grid.structures);

                if (this.uiCallbacks.showDiscovery) {
                    this.uiCallbacks.showDiscovery('Massive Discovery', 2000);
                }
